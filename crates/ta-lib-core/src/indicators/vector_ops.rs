...
pub(crate) fn binary_op(
    ...
    *out_nb_element = 0;
    ...
    if let Err(ret_code) = validate_input_len(in_real0, end_idx) {
        return ret_code;
    }
    if let Err(ret_code) = validate_input_len(in_real1, end_idx) {
        return ret_code;
    }
    ...
    for (out_idx, index) in (start_idx..=end_idx).enumerate() {
        out_real[out_idx] = op(in_real0[index], in_real1[index]);
    }
